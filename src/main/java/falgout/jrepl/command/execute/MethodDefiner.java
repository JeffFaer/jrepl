...
public class MethodDefiner extends RepositoryDefiner<MethodDeclaration, Method> {
    private static final ThrowingBiFunction<Environment, MethodDeclaration, NamedSourceCode<? extends Method>, ClassNotFoundException> CONVERTOR = new ThrowingBiFunction<Environment, MethodDeclaration, NamedSourceCode<? extends Method>, ClassNotFoundException>() {
        @Override
                    ...
                    Collection<? extends NamedSourceCode<? extends Method>> others = repo.getCode(name)
                            ...
                            .map(c -> {
                                ...
                                if (c instanceof MethodSourceCode) {
                                    ...
                                    throw new AssertionError("We've only put {Overloaded}MethodSourceCode in here,"
                                            + " this shouldn't happen");
                                }
                                
                                // make the overloaded method private to prevent ambiguous
                                // imports
